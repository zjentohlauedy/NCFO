...
class Stats
  ...
  def to_s
    sprintf "%s %s %-20s %-15s %4d", @when, @pos, @name, @school, get_sort_key.to_i
  end
...
class Qbr < Stats
  ...
  def to_s
    sprintf "%s %s %-20s %-15s %7.2f", @when, @pos, @name, @school, get_sort_key.to_f
  end
  ...
  def process_categories( categories )
    categories.each do |key, value|
      ...
      if players.nil?
        ...
        value.store 'players', players + @players
      end
    end
  end
